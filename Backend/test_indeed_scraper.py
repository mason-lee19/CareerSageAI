...
class TestScraper(unittest.TestCase):
    ...
    def _assert_job_listing(actual: JobListing, expected: JobListing):
        ...
        if expected.jobTitle != actual.jobTitle:
            errors.append(f"jobTitle mismatch: {expected.jobTitle} != {actual.jobTitle}")
        if expected.jobCompany != actual.jobCompany:
            errors.append(f"jobCompany mismatch: {expected.jobCompany} != {actual.jobCompany}")
        ...
        if errors:
            error_message = "\n".join(errors)
            raise AssertionError(f"JobListing validation failed with the following mismatches:\n{error_message}")
