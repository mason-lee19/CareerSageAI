...
class DataBase():
    ...
    def _job_exists(self,job_key: str,) -> bool:
        """
    ...
    def add_job(self,job_data) -> None:
        ...
        for idx in range(len(job_data.jobLink)):
            ...
            if job_data.expirationStatus[idx]:
                logger.info(f'[DB] Job has expired removing/skipping {job_data.jobTitle} - {job_data.jobCompany}')
            ...
            if not self._job_exists(job_key):
                ...
                logger.info(f'[DB] Job {job_data.jobTitle} already exists in the database')
